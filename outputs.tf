...
output "eventgrid_topics_identity" {
  ...
  value       = { for k, v in azurerm_eventgrid_topic.eventgrid_topics : k => one(v.identity) if v.identity != null && length(v.identity) > 0 }
}
...
output "eventgrid_topics_input_mapping_default_values" {
  ...
  value       = { for k, v in azurerm_eventgrid_topic.eventgrid_topics : k => one(v.input_mapping_default_values) if v.input_mapping_default_values != null && length(v.input_mapping_default_values) > 0 }
}
output "eventgrid_topics_input_mapping_fields" {
  ...
  value       = { for k, v in azurerm_eventgrid_topic.eventgrid_topics : k => one(v.input_mapping_fields) if v.input_mapping_fields != null && length(v.input_mapping_fields) > 0 }
}
